  ...
  const readFile = useCallback(
    async (file: File) => {
      try {
        ...
        const sheet =
          pickSheetByName(ing, /roadmap.*(q3|q4)|(q3|q4).*roadmap/i) ??
          pickSheetByName(ing, /roadmap/i) ??
          ing.sheets[0];
  ...
  return (
    ...
    >
      ...
      <div className="text-sm font-medium text-foreground">
        Drop hires CSV or Excel here
      </div>
      <div className="text-xs text-muted-foreground">
        Accepts CSV, TSV, or Excel exported from Google Sheets, Excel, or any spreadsheet app.
      </div>
      <div className="text-xs text-muted-foreground">
      ...
      <input
        ref={inputRef}
        type="file"
        accept=".csv,.tsv,.txt,.xlsx,.xls,.pdf,text/csv,text/tab-separated-values,text/plain,application/csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/pdf,application/octet-stream"
        className="hidden"
        onChange={(e) => {
